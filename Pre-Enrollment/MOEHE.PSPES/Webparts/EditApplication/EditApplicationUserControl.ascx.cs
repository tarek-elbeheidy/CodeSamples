...
namespace MOEHE.PSPES.Webparts.EditApplication
{
    public class test
    {
    ...
    public partial class EditApplicationUserControl : UserControl
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            {
                ApplicationReferenceNumber = Request.QueryString["ApplicationRefNumber"];
            ...
            try
            {
                if (!Page.IsPostBack)
                {
                    ...
                    {
                        GetApplicationData(ApplicationReferenceNumber, QID);
                    }
                    ...
                    else
                    {
                        ...
                        if ((uint)CultureInfo.CurrentUICulture.LCID == 1025)
                        {
                            errorMessage = PSPESConstants.ArabicInvalidRefernceNumber;
                        }
                        else
                        {
                            errorMessage = PSPESConstants.EnglishInvalidRefernceNumber;
                        }
                        txtQID.Enabled = false;
                        CustomMessageBoxHelper.Show(this.Page, errorMessage);
        ...
        private void GetApplicationData(string ApplicationReferenceNumber, string QID)
        {
            ...
            if (ApplicationReferenceNumber.Length > 0)
            {
                MOE_APPLICATION_DATA_Model appDTModel = MOE_APPLICATION_DATA_Repository.GetByRefAndID(ApplicationReferenceNumber, QID).Result;
                ...
                {
                    List<SeatReservationFee> finalizedApplication = SeatReservationFeeRepository.GetbyRefAndID(ApplicationReferenceNumber, QID).Result;

                    if (finalizedApplication.Count > 0)
                    {
                        if ((uint)CultureInfo.CurrentUICulture.LCID == 1025)
                        { CustomMessageBoxHelper.Show(this.Page, "هذا الطلب تم استكماله وتم حجز المقعد ولا يمكن تعديله"); }
                        else { CustomMessageBoxHelper.Show(this.Page, "This Application has been finalized , seat was reserved and application can not be edited"); }
                    }
                    #region fill application data to be edited
                    else
                    {

                        #region  Bind Student Bio Information from MOI
                        MOE_BIO_DATA_Model Bio_Model = MOE_BIO_DATA_Repository.GetBioDataByQID(decimal.Truncate((decimal)appDTModel.NATIONAL_ID).ToString()).Result;
                        if (Bio_Model != null)
                        {
                            if ((uint)CultureInfo.CurrentUICulture.LCID == 1025)
                            {
                            ...
                            txtQID.Text = Convert.ToInt64(Bio_Model.NATIONAL_ID).ToString();
                        ...
                        txtApplicationRefNo.Text = appDTModel.MOE_APPL_REF_NBR;
                        dtApplicationDate2.Value = ((DateTime)appDTModel.MOE_APPL_DATE).ToString("dd/MM/yyyy", CultureInfo.GetCultureInfo(2057));
                        ddlPreEnrollmentTerm.SelectedValue = ((int)appDTModel.MOE_TERM).ToString();

                        #region Load Preenrollment School Data

                        if (ViewState["schoolCode"] != null)
                            ViewState["schoolCode"] = appDTModel.MOE_SCHOOL_CODE;
                        else
                        {

                            ViewState.Add("schoolCode", appDTModel.MOE_SCHOOL_CODE);

                        }
                        //schoolCode = appDTModel.MOE_SCHOOL_CODE;
                        getSchoolData(appDTModel.MOE_SCHOOL_CODE);

                        #endregion
                        /*

                        MOE_SCHOOL_Model schoolInfo = MOE_SCHOOL_Repository.GetSchoolInfo(DateTime.Now.Year.ToString(), appDTModel.MOE_SCHOOL_CODE, "false").Result;
                        if (schoolInfo != null)
                        {
                            if ((uint)CultureInfo.CurrentUICulture.LCID == 1025)
                            {
                                txtPreEnrollmentSchool.Text = schoolInfo.MOE_SCHOOL_NAME_ARA;
                            }
                            else
                            {
                                txtPreEnrollmentSchool.Text = schoolInfo.MOE_SCHOOL_NAME_ENG;
                            }
                        ...
                        foreach (ListItem item in ddlPreEnrolmentGrade.Items)
                        {
                            ...
                            index++;
                            MOE_AGE_VALIDATE_Model Age_Model = MOE_AGE_VALIDATE_Repository.GetGradeAgeRange(ddlPreEnrollmentTerm.SelectedValue, getSchoolCode(), "Private", lblPreEnrollmentSchoolCurriculumID.Text, item.Value.Split(';')[0]).Result;
                            if ((uint)CultureInfo.CurrentUICulture.LCID == 1025)
                            {
                            ...
                            if (item.Value.Split(';')[0] == appDTModel.MOE_APPLIED_GRADE)
                            {
                        ...
                        ddlPreEnrolmentGrade.SelectedIndex = selectedGradeIndex;


                        txtResedentialArea.Text = appDTModel.MOE_RESIDENTIAL_AREA;
                        ddlTransportation.SelectedValue = ((bool)appDTModel.MOE_AVAIL_TRANSPORT) ? "true" : "false";
                        txtWaitListNumber.Value = appDTModel.MOE_WAITLIST_NUMBER.Value.ToString();


                        /* }*/
                        #endregion

                        #region get and bind academic Information
                        List<MOE_APPL_ACAD_INFO_Model> unorderedAcadInfo = MOE_APPL_ACAD_INFO_Repository.GetAcadInfoRefernceNumber(ApplicationReferenceNumber).Result;

                        if (unorderedAcadInfo != null && unorderedAcadInfo.Count > 0)
                        {
                            List<MOE_APPL_ACAD_INFO_Model> orderedAcadInfo = unorderedAcadInfo.OrderByDescending(D => D.MOE_TERM).ToList();
                            #region Load current year data
                            txtCurrentSchoolCode.Text = orderedAcadInfo[0].MOE_SCHOOL_CODE;
                            MOE_SCHOOL_Model currentSchoolInfo = MOE_SCHOOL_Repository.GetSchoolInfo(orderedAcadInfo[0].MOE_TERM.ToString(), orderedAcadInfo[0].MOE_SCHOOL_CODE, "false").Result;
                            if (currentSchoolInfo != null)
                            {
                                if ((uint)CultureInfo.CurrentUICulture.LCID == 1025)
                                {
                                    txtCurrentSchool.Text = currentSchoolInfo.MOE_SCHOOL_NAME_ARA;
                                }
                                else
                                {
                                    txtCurrentSchool.Text = currentSchoolInfo.MOE_SCHOOL_NAME_ENG;
                                }

                                ddlCurrentCurriculum.SelectedValue = currentSchoolInfo.MOE_SCHOOL_CURRICULUM_ID;


                                LoadGrades(orderedAcadInfo[0].MOE_TERM.ToString(), currentSchoolInfo.MOE_SCHOOL_CODE, ddlCurrentGrade, ddlCurrentCurriculum.SelectedValue);


                                // ddlCurrentGrade.SelectedValue = CurrentAcad_Model.MOE_GRADE;
                                ddlCurrentGrade.Items.Cast<ListItem>()
                                    .Where(x => x.Value.Contains(orderedAcadInfo[0].MOE_GRADE))
                                    .LastOrDefault().Selected = true;

                                //txtCurrentGrade.Text = CurrentAcad_Model.MOE_GRADE.ToString();

                                if (orderedAcadInfo[0].MOE_STU_RSLT == "True")//ask about status values from NSIS
                                {
                            ...
                            if (unorderedAcadInfo.Count > 1)
                            {
                                #region bind previous academic information
                                txtPreviousSchoolCode.Text = orderedAcadInfo[1].MOE_SCHOOL_CODE;
                                MOE_SCHOOL_Model previousSchoolInfo = MOE_SCHOOL_Repository.GetSchoolInfo(orderedAcadInfo[1].MOE_TERM.ToString(), orderedAcadInfo[1].MOE_SCHOOL_CODE, "false").Result;
                                if (previousSchoolInfo != null)
                                {
                                    if ((uint)CultureInfo.CurrentUICulture.LCID == 1025)
                                    {
                                        txtPreviousSchool.Text = previousSchoolInfo.MOE_SCHOOL_NAME_ARA;
                                    }
                                    else
                                    {
                                        txtPreviousSchool.Text = previousSchoolInfo.MOE_SCHOOL_NAME_ENG;
                                    }

                                    ddlPreviousCurriculum.SelectedValue = previousSchoolInfo.MOE_SCHOOL_CURRICULUM_ID;

                                    LoadGrades(orderedAcadInfo[1].MOE_TERM.ToString(), previousSchoolInfo.MOE_SCHOOL_CODE, ddlPreviousGrade, ddlPreviousCurriculum.SelectedValue);


                                    // ddlCurrentGrade.SelectedValue = CurrentAcad_Model.MOE_GRADE;
                                    ddlPreviousGrade.Items.Cast<ListItem>()
                                        .Where(x => x.Value.Contains(orderedAcadInfo[1].MOE_GRADE))
                                        .LastOrDefault().Selected = true;

                                    //txtCurrentGrade.Text = CurrentAcad_Model.MOE_GRADE.ToString();

                                    if (orderedAcadInfo[1].MOE_STU_RSLT == "True")//ask about status values from NSIS
                                    {
                ...
                if (studentContacts.Count > 0)
                {
                    ...
                    if (guardian != null)
                    {
                        ...
                        GuardianQID_Old.Value = Convert.ToInt64(guardian.bio_data.NATIONAL_ID).ToString();
                        ...
                        txtGuardianMobile.Text = guardian.MOE_MOBILE_CONTACT_NBR;
                ...
                #region get andBind Student Health Information 

                MOE_HEALTH_DATA_Model health_Model = MOE_HEALTH_DATA_Repository.GetHealthInfoByRefernceNumber(ApplicationReferenceNumber).Result;
                if (health_Model != null)
                {
                    ...
                    if (health_Model.MOE_SPL_NEEDS.HasValue)
                    {
                        string Value = health_Model.MOE_SPL_NEEDS.Value == true ? "true" : "false";
                        ddlSpecialNeed.Items.FindByValue(Value).Selected = true;
                        specialneedValid.Value = Value;
                    }
                    ...
                    if (health_Model.MOE_LEARNING_DIFFICULTIES.HasValue)
                    {
                        string Value = health_Model.MOE_LEARNING_DIFFICULTIES.Value == true ? "true" : "false";
                        ddlLearningDifficulties.Items.FindByValue(Value).Selected = true;
                        learningdifficultiesValid.Value = Value;
                    }

                    //bool learningDifficulties = health_Model.MOE_LEARNING_DIFFICULTIES.HasValue ? health_Model.MOE_LEARNING_DIFFICULTIES.Value : false;
                ...
                #region get & bind Application Supporting Documents
                bool isMinistryUser = true;
                if ((uint)CultureInfo.CurrentUICulture.LCID == 1025)
                {
                    BindingUtility.LoadRequiredSupportingDocs(gvRequiredDocuments, ApplicationReferenceNumber, Convert.ToDecimal(txtQID.Text), new SupportingDocsModel { SchoolCode = appDTModel.MOE_SCHOOL_CODE, Term = appDTModel.MOE_TERM.ToString(), Grade = appDTModel.MOE_APPLIED_GRADE, SeacrhByTermAndSchoolCodeAndGrade = true }, false, isMinistryUser, SPContext.Current.Site.Url);
        ...
        private void getSchoolData(string schoolCode)
        {
            try
            {
                ...
                MOE_SCHOOL_Model schoolInfo = MOE_SCHOOL_Repository.GetSchoolInfo(TermRepository.GetTerms().Result.Where(s => s.ACADEMIC_YEAR_DESC == "CURRENT").Select(d => d).FirstOrDefault().TermCode, schoolCode, "false").Result;
                ...
                if (schoolInfo != null)
                {
                    ...
                    {
                        DBOperationResult ReturnedResult2 = TransactionLogsRepository.Insert(new TransactionsLog { CreatedDate = DateTime.Now, ShortDescription = "Edit Application Control", Description = "Edit Application getSchoolData NSIS Exception: " + ex.Message, UserID = SPContext.Current.Site.RootWeb.CurrentUser.LoginName }).Result;
                    ...
                    /*
                     ...
                     {
                         DBOperationResult ReturnedResult2 = TransactionLogsRepository.Insert(new TransactionsLog { CreatedDate = DateTime.Now, ShortDescription = "Edit Application Control", Description = "Add Application getSchoolData School Grade-Age Mapping Exception, CurriculumID: " + lblPreEnrollmentSchoolCurriculumID.Text + ", Exception:" + ex.Message, UserID = SPContext.Current.Site.RootWeb.CurrentUser.LoginName }).Result;
                         if ((uint)CultureInfo.CurrentUICulture.LCID == 1033)
                         {
                ...
                {
                    DBOperationResult ReturnedResult2 = TransactionLogsRepository.Insert(new TransactionsLog { CreatedDate = DateTime.Now, ShortDescription = "Edit Application Control", Description = "Edit Application getSchoolData NSIS school Null Exception, school code : " + schoolCode, UserID = SPContext.Current.Site.RootWeb.CurrentUser.LoginName }).Result;
                    if ((uint)CultureInfo.CurrentUICulture.LCID == 1033)
                    {
                        CustomMessageBoxHelper.Show(this.Page, "No School Information returned from NSIS, please contact helpdesk");
            ...
            {
                //DBOperationResult ReturnedResult2 = TransactionLogsRepository.Insert(new TransactionsLog { CreatedDate = DateTime.Now, ShortDescription = "Edit Application Control", Description = "Add Application getSchoolData Exception: " + ex.Message, UserID = SPContext.Current.Site.RootWeb.CurrentUser.LoginName }).Result;
                //if ((uint)CultureInfo.CurrentUICulture.LCID == 1033)
                //{
        ...
        private void BindRequiredSupportedDocuments(string requestedGrade)
        {
            try
            {
                ...
                bool isMinistryUser = true;
                MOE_SCHOOL_Model SchoolInfo = MOE_SCHOOL_Repository.GetSchoolInfo(Term, getSchoolCode(), "false").Result;
            ...
            try
            {
                ...
                else
                {
                    ...
                    if (enrollment_controlling_model != null && enrollment_controlling_model.SchoolCode == schoolCode)
                    {
                        ...
                        else if (studentCountryCode != "-1" && !(enrollment_controlling_model.CountryCodes.Contains(studentCountryCode) || enrollment_controlling_model.CountryCodes == "ALL"))
                        {
                        ...
                        else if (!(enrollment_controlling_model.GradeCodes.Contains(ddlPreEnrolmentGrade.SelectedValue.Split(';')[0]) || enrollment_controlling_model.GradeCodes == "ALL"))
                        {
        ...
        private bool ValidateAge(string term, string schoolType, string schoolCurriculum, string selectedGrade, int selectedGradeWeight, int currentGradeWeigh)
        {
            ...
            if (validate_age_model != null)
            {
                ...
                int StudentAge = Age(cut_off_date, studentDOB);
        ...
        public static int Age(DateTime cut_off_date, DateTime studentDOB)
        {

            int age = cut_off_date.Year - studentDOB.Year;
        ...
        private void LoadPreEnrolmentGradeData(string selectedGrade)
        {
            try
            {
                ...
                //txtWaitListNumber.Value = (MOE_APPLICATION_DATA_Repository.GetCurrentWaitListNumber(int.Parse(ddlPreEnrollmentTerm.SelectedValue), schoolCode, ddlPreEnrolmentGrade.SelectedItem.Value.Split(';')[0]).Result + 1).ToString();
        ...
        private void SaveApplication()
        {
            ...
            try
            {
                ...
                string Application_Reference_Number = txtApplicationRefNo.Text;//ddlPreEnrollmentTerm.SelectedValue + "-" + schoolCode + "-" + ddlPreEnrolmentGrade.SelectedValue.Split(';')[0] + "-" + txtWaitListNumber.Value;
                DBOperationResult ReturnedResult2 = TransactionLogsRepository.Insert(new TransactionsLog { CreatedDate = DateTime.Now, ShortDescription = "Edit Application Control", Description = "Start Save Application Data", UserID = SPContext.Current.Site.RootWeb.CurrentUser.LoginName }).Result;
                try
                {
                    ...
                    ReturnedResult2 = TransactionLogsRepository.Insert(new TransactionsLog { CreatedDate = DateTime.Now, ShortDescription = "Edit Application Control", Description = "finish Save Application Data", UserID = SPContext.Current.Site.RootWeb.CurrentUser.LoginName }).Result;
                }
                catch (Exception ex)
                {
                    ReturnedResult2 = TransactionLogsRepository.Insert(new TransactionsLog { CreatedDate = DateTime.Now, ShortDescription = "Edit Application Control", Description = "Save Application Data Exception: " + ex.Message, UserID = SPContext.Current.Site.RootWeb.CurrentUser.LoginName }).Result;
                    string confirmationMessage = "";
                ...
                try
                {
                    ...
                    {
                        ReturnedResult2 = TransactionLogsRepository.Insert(new TransactionsLog { CreatedDate = DateTime.Now, ShortDescription = "Edit Application Control", Description = "Start Save Application Current Academic Data", UserID = SPContext.Current.Site.RootWeb.CurrentUser.LoginName }).Result;
                ...
                {
                    ReturnedResult2 = TransactionLogsRepository.Insert(new TransactionsLog { CreatedDate = DateTime.Now, ShortDescription = "Edit Application Control", Description = "Save Application current academic information Data Exception: " + ex.Message, UserID = SPContext.Current.Site.RootWeb.CurrentUser.LoginName }).Result;
                    string confirmationMessage = "";
                ...
                try
                {
                    ...
                    ReturnedResult2 = TransactionLogsRepository.Insert(new TransactionsLog { CreatedDate = DateTime.Now, ShortDescription = "Edit Application Control", Description = "Save Application Previous Academic Information Data", UserID = SPContext.Current.Site.RootWeb.CurrentUser.LoginName }).Result;
                ...
                {
                    ReturnedResult2 = TransactionLogsRepository.Insert(new TransactionsLog { CreatedDate = DateTime.Now, ShortDescription = "Edit Application Control", Description = "Save Application Previous Academic Information Data Exception: " + ex.Message, UserID = SPContext.Current.Site.RootWeb.CurrentUser.LoginName }).Result;
                    string confirmationMessage = "";
                ...
                try
                {
                    ...
                    ReturnedResult2 = TransactionLogsRepository.Insert(new TransactionsLog { CreatedDate = DateTime.Now, ShortDescription = "Edit Application Control", Description = "Save Application Guardian Data", UserID = SPContext.Current.Site.RootWeb.CurrentUser.LoginName }).Result;
                    ...
                    if (txtGuardianQID.Text != "")
                    {
                        MOE_GUARDIAN_DATA_Model_Not_Serializable guardian_model = new MOE_GUARDIAN_DATA_Model_Not_Serializable()
                        {
                            ...
                            MOE_TRANSACTION_DTTM = DateTime.Now,
                            MOE_USERID = SPContext.Current.Site.RootWeb.CurrentUser.LoginName,
                            MOE_OLD_RELATED_QID = (GuardianQID_Old.Value != "") ? decimal.Parse(GuardianQID_Old.Value) : 0,
                ...
                {
                    ReturnedResult2 = TransactionLogsRepository.Insert(new TransactionsLog { CreatedDate = DateTime.Now, ShortDescription = "Edit Application Control", Description = "Save Application Guardian Data Exception: " + ex.Message, UserID = SPContext.Current.Site.RootWeb.CurrentUser.LoginName }).Result;
                    string confirmationMessage = "";
                ...
                try
                {
                    ...
                    ReturnedResult2 = TransactionLogsRepository.Insert(new TransactionsLog { CreatedDate = DateTime.Now, ShortDescription = "Edit Application Control", Description = "Save Application Student Contacts Data ", UserID = SPContext.Current.Site.RootWeb.CurrentUser.LoginName }).Result;
...

                    {
                        ...
                        foreach (MOE_GUARDIAN_DATA_Model contact in Contacts)
                        {
                            MOE_GUARDIAN_DATA_Model_Not_Serializable NS_guardian_model = new MOE_GUARDIAN_DATA_Model_Not_Serializable()
                            {
                                ...
                                MOE_TRANSACTION_DTTM = DateTime.Now,
                                MOE_USERID = SPContext.Current.Site.RootWeb.CurrentUser.LoginName,
                                MOE_OLD_RELATED_QID = (txtGuardianQID.Text != "") ? decimal.Parse(txtGuardianQID.Text) : 0,
                                MOE_GUARDIAN_NAME_ARA = arabicguardianname.Value,
                                MOE_GUARDIAN_NAME_ENG = englishguardianname.Value
                ...
                {
                    ReturnedResult2 = TransactionLogsRepository.Insert(new TransactionsLog { CreatedDate = DateTime.Now, ShortDescription = "Edit Application Control", Description = "Save Application Student Contacts Data Exception: " + ex.Message, UserID = SPContext.Current.Site.RootWeb.CurrentUser.LoginName }).Result;
                    string confirmationMessage = "";
                ...
                try
                {
                    ...
                    ReturnedResult2 = TransactionLogsRepository.Insert(new TransactionsLog { CreatedDate = DateTime.Now, ShortDescription = "Edit Application Control", Description = "Save Application Health Data", UserID = SPContext.Current.Site.RootWeb.CurrentUser.LoginName }).Result;
                ...
                {
                    ReturnedResult2 = TransactionLogsRepository.Insert(new TransactionsLog { CreatedDate = DateTime.Now, ShortDescription = "Edit Application Control", Description = "Save Application Health Data Exception: " + ex.Message, UserID = SPContext.Current.Site.RootWeb.CurrentUser.LoginName }).Result;
                    string confirmationMessage = "";
                ...
                try
                {
                    ...
                    ReturnedResult2 = TransactionLogsRepository.Insert(new TransactionsLog { CreatedDate = DateTime.Now, ShortDescription = "Edit Application Control", Description = "Save Application Documents Data ", UserID = SPContext.Current.Site.RootWeb.CurrentUser.LoginName }).Result;
                ...
                {
                    ReturnedResult2 = TransactionLogsRepository.Insert(new TransactionsLog { CreatedDate = DateTime.Now, ShortDescription = "Edit Application Control", Description = "Save Application Documents Data Exception: " + ex.Message, UserID = SPContext.Current.Site.RootWeb.CurrentUser.LoginName }).Result;
                    string confirmationMessage = "";
                ...
                ReturnedResult2 = TransactionLogsRepository.Insert(new TransactionsLog
                {
                    ...
                    ShortDescription = "Edit Application Control",
                   ...
                   ApplicationSupportDocsResult.EnglishResult == PSPESConstants.InsertionDone*/ )
                {
                    ...
                    if ((uint)CultureInfo.CurrentUICulture.LCID == 1025)
                    {
                        ...
                        confirmationMessage = "تم تعديل الطلب بنجاح";
                    }
                    else
                    {

                        confirmationMessage = "Application has been edited successfully";
                    }
                    CustomMessageBoxHelper.Show(this.Page, confirmationMessage);
                    return;
                ...
                else
                {
                    ...
                    if ((uint)CultureInfo.CurrentUICulture.LCID == 1025)
                    {
                        ...
                        confirmationMessage = "حدث خطأ أثناء الحفظ";
                    }
                    else
                    {

                        confirmationMessage = "An error occurred while saving the application";
                    }
                    CustomMessageBoxHelper.Show(this.Page, confirmationMessage);
                    return;
                }
            ...
            catch (Exception ex)
            {
                ...
                DBOperationResult ReturnedResult2 = TransactionLogsRepository.Insert(new TransactionsLog { CreatedDate = DateTime.Now, ShortDescription = "Edit Application Control", Description = "Save Application Data General Exception: " + ex.Message, UserID = SPContext.Current.Site.RootWeb.CurrentUser.LoginName }).Result;
                return;
        ...
        {
            SendSMS.SendMessage("test title", "33493587", "this is testing");
        ...
        /*private void LoadGrades(string schoolYearID, DropDownList ddlGrade, string selectedCurriculum)
        {
            ...
            List<MOE_AGE_VALIDATE_Model> AgeValidateModelCurriculumGrades = MOE_AGE_VALIDATE_Repository.GetGradeList(ddlPreEnrollmentTerm.SelectedValue,getSchoolCode(), "Private", selectedCurriculum).Result;
        ...
        private void GuardianQIDChanged()
        {
            try
            {
                if (txtGuardianQID.Text.Length > 0)
                {
