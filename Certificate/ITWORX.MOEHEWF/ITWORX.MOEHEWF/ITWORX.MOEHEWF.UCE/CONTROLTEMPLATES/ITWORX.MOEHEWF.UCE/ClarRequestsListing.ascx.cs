    ...
    public partial class ClarRequestsListing : UserControlBase
    {
                    ...
                    else
                    {
                        if (Page.Session["DisplayRequestId"] != null)
                        {
                            ...
                            if ((Utilities.BusinessHelper.GetAssignee(Page.Session["DisplayRequestId"].ToString()) &&
                                ...
                                (Utilities.BusinessHelper.GetAssignee(Page.Session["DisplayRequestId"].ToString()) && StatusId.LookupId != 37 &&
        ...
        private void BindGrid(string ReqID)
        {
            try
            {
                ...
                List<ClarificationReqs> ClarRequests = BL.ClarificationRequests.GetClarificationRequestbyReqID(ReqID);
        ...
        protected void lnk_View_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (!IsRefresh)
                {
                    ...
                    //Response.Redirect((LCID == 1033 ? "/en" : "/ar") + "/Pages/ClarificationReqDetails.aspx");
        ...
        protected void lnk_ReplytoClarification_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (HelperMethods.InGroup(Common.Utilities.Constants.ReceptionistGroupName))
                    Response.Redirect(SPContext.Current.Web.Url + "/_layouts/15/ITWORX.MOEHEWF.UCE~/Receptionist.aspx");
        ...
        protected void grd_ClarRequests_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            try
            {
                ...
                if (e.Row.RowType == DataControlRowType.DataRow)
                {
                    ...
                    if (isExist)
                    {
                        if (string.IsNullOrEmpty(lbl_ClarReply.Text) && string.IsNullOrEmpty(lbl_ReplyDate.Text))
                        {
        ...
        {
            Session["ActionTaken"] = "AddNewClarification";
            //string ReceptionistURL = SPContext.Current.Web.Url + "/_layouts/15/ITWORX.MOEHEWF.UCE/Receptionist.aspx";
