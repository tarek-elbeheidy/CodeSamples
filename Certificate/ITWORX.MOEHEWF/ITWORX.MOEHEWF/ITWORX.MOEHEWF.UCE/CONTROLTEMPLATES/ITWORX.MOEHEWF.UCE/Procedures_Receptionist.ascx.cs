    ...
    public partial class Procedures_Receptionist : UserControlBase
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                if (Page.Session["DisplayRequestId"] != null)
                {
                    ...
                    if (Utilities.BusinessHelper.GetAssignee(Page.Session["DisplayRequestId"].ToString()) && HelperMethods.InGroup(Common.Utilities.Constants.ReceptionistGroupName))
        ...
        private void BindGrid(string ReqID)
        {
            try
            {
                ...
                List<Entities.Procedures> Procedures = BL.AllProcedures.GetProceduresbyReqID(Utilities.Constants.ReceptionProcedures, ReqID);
        ...
        protected void lnk_View_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                if (!IsRefresh)
                {
                    ...
                    SPUtility.Redirect(SPContext.Current.Web.Url + "/Pages/ProcedureReceptionistDetails.aspx", SPRedirectFlags.DoNotEndResponse, HttpContext.Current);
                }
            }
            catch (Exception ex)
            {
