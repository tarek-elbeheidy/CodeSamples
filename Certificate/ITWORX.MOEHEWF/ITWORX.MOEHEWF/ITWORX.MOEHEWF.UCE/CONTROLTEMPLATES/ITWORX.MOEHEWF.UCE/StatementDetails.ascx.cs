...
namespace ITWORX.MOEHEWF.UCE.CONTROLTEMPLATES.ITWORX.MOEHEWF.UCE
{
    public partial class StatementDetails : UserControlBase
    {
        ...
        protected void btn_Close_Click(object sender, EventArgs e)
        {
            try
            {
                if (!IsRefresh)
                {
                   ...
                   // Response.Redirect((LCID == 1033 ? "/en" : "/ar") + "/Pages/StatementRequestsListing.aspx");
        ...
        private void BindAttachements()
        {
            ...
            #region Display Mode
            int requestId = 0;
            if (Page.Session["DisplayRequestId"] != null && Page.Session["hdn_StatementReqId"] != null)
            {
                requestId = int.Parse(Convert.ToString(Page.Session["DisplayRequestId"]));
            }
