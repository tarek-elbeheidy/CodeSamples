...
{
    public partial class ViewSimilarRequests : UserControlBase
    {
        ...
        {
            Logging.GetInstance().Debug("Enter ViewSimilarRequests.lnk_Request_Click");
            try
            {
                List<Entities.SimilarRequest> SimilarRequests = BL.SearchSimilarRequests.GetSimilarRequstsbyReqId(BL.SearchSimilarRequests.GetCheckedRequests(int.Parse(Page.Session["DisplayRequestId"].ToString())), LCID).ToList();
            ...
            {
                Logging.GetInstance().Debug("Exit ViewSimilarRequests.lnk_Request_Click");
            }
        }
