...
{
    public partial class Procedures_DeptManagerListing : UserControlBase
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                if (Page.Session["DisplayRequestId"] != null)
                {
                    ...
                    if (Utilities.BusinessHelper.GetAssignee(Page.Session["DisplayRequestId"].ToString()) && HelperMethods.InGroup(Common.Utilities.Constants.DepartmentManagerGroupName))
        ...
        private void BindGrid(string ReqID)
        {
            try
            {
                Logging.GetInstance().Debug("Enter Procedures_DeptManagerListing.BindGrid");
                List<Entities.Procedures> Procedures = BL.AllProcedures.GetProcedurebyReqID(Utilities.Constants.DepartmentManagerProcedures, ReqID);
            ...
            {
                Logging.GetInstance().Debug("Exit Procedures_DeptManagerListing.BindGrid");
            }
        }
        protected void grd_Procedures_PageIndexChanging(object sender, GridViewPageEventArgs e)
        {
            Logging.GetInstance().Debug("Enter Procedures_DeptManagerListing.grd_Procedures_PageIndexChanging");
            try
            {
            ...
            finally
            {
                ...
                Logging.GetInstance().Debug("Exit Procedures_DeptManagerListing.grd_Procedures_PageIndexChanging");
            }
        }
        protected void lnk_View_Click(object sender, EventArgs e)
        {
            Logging.GetInstance().Debug("Enter Procedures_DeptManagerListing.lnk_View_Click");
            try
            {
                if (!IsRefresh)
                {
                    ...
                    SPUtility.Redirect(SPContext.Current.Web.Url + "/Pages/ProcedureDepartManagerDetails.aspx", SPRedirectFlags.DoNotEndResponse, HttpContext.Current);
                }
            }
            catch (Exception ex)
            {
            ...
            {
                Logging.GetInstance().Debug("Exit Procedures_DeptManagerListing.lnk_View_Click");
            }
        }
