                ...
                using (SPSite site = new SPSite(SPContext.Current.Site.Url))
                {
                    using (SPWeb web = site.OpenWeb())
                    {
                        try
                        {
                            ...
                            SPList list = web.Lists[Utilities.Constants.ProgramManagerRecommendation];
            ...
            SPSecurity.RunWithElevatedPrivileges(delegate ()
            {
                using (SPSite site = new SPSite(SPContext.Current.Site.Url))
                {
                    using (SPWeb web = site.OpenWeb())
                    {
                        try
                        {
                            Logging.GetInstance().Debug("Entering method Recommendations.GetRecommendationbyReqID");
                            SPList list = web.Lists[Utilities.Constants.ProgramManagerRecommendation];

                            var q = Common.Utilities.BusinessHelper.GetQueryObject("<Where><Eq><FieldRef Name='RequestID' /><Value Type='Lookup'>" +
                                ReqID + "</Value></Eq></Where><OrderBy><FieldRef Name='RecommendationDate' Ascending='False' /></OrderBy>");
                        ...
                        {
                            Logging.GetInstance().Debug("Exiting method Recommendations.GetRecommendationbyReqID");
                        }
                    }
                }
            });
