...
namespace ITWORX.MOEHEWF.UCE.BL
{
    public class ExternalCommunications
    {
            ...
            using (SPSite site = new SPSite(SPContext.Current.Site.Url))
            {
                using (SPWeb web = site.OpenWeb())
                {
                    try
                    {
                        ...
                        SPList list = web.Lists[Utilities.Constants.ExternalCommunications];
                    ...
                    using (SPWeb web = site.OpenWeb())
                    {
                        try
                        {
                            ...
                            if (item != null)
                            {
                                ...
                                {
                                    var BookTextField = list.Fields.GetFieldByInternalName("BookText");
                                    var booktxt = item[BookTextField.Id];
                                    ExtComms.BookText = BookTextField.GetFieldValueAsText(booktxt);
                                }
                                else
                                    ExtComms.BookText = string.Empty;
        ...
        public static void UpdateBookbyOrgReply(string id, string reply,string OrgbookNo,string OrgReplyBookSubject)
        {
            SPSecurity.RunWithElevatedPrivileges(delegate ()
            {
                using (SPSite site = new SPSite(SPContext.Current.Site.Url))
                {
                    using (SPWeb web = site.OpenWeb())
                    {
                        try
                        {
                            ...
                            itemToUpdate["OrgReplyBookSubject"] = SPHttpUtility.HtmlEncode(OrgReplyBookSubject);
        ...
        public static List<ExternalComms> GetAllOrganizations()
        {
            List<ExternalComms> external = new List<ExternalComms>();
            SPSecurity.RunWithElevatedPrivileges(delegate ()
            {
                using (SPSite site = new SPSite(SPContext.Current.Site.Url))
                {
                    using (SPWeb web = site.OpenWeb())
                    {
                        try
                        {
                            Logging.GetInstance().Debug("Entering method ExternalCommunications.GetAllOrganizations");
                            SPList list = web.Lists[Utilities.Constants.BookOrganizationTemp];
                            SPListItemCollection collListItems = list.GetItems();
                            if (collListItems.Count != 0)
                                foreach (SPListItem item in collListItems)
                                {
                                    external.Add(new ExternalComms()
                                    {
                                        OrgTitleAr = item["OrgTitleAr"] == null ? string.Empty : item["OrgTitleAr"].ToString(),
                                        ID = item["ID"] == null ? string.Empty : item["ID"].ToString(),
                                        OrgTitleEN = item["OrgTitleEN"] == null ? string.Empty : item["OrgTitleEN"].ToString(),
                                    });
                        ...
                        {
                            //dispose any objects that require dispose.
                            Logging.GetInstance().Debug("Exiting method ExternalCommunications.GetAllOrganizations");
                        }
                    }
                }
            });
        ...
        public static ExternalComms GetOrgTemplatebyID(int ID)
        {
            ExternalComms Org = new ExternalComms();
            SPSecurity.RunWithElevatedPrivileges(delegate ()
            {
                using (SPSite site = new SPSite(SPContext.Current.Site.Url))
                {
                    using (SPWeb web = site.OpenWeb())
                    {
                        try
                        {
                            ...
                            if (Item != null)
                            {
                                Org.TemplateEN = Item["TemplateEN"] == null ? string.Empty : Item["TemplateEN"].ToString();
                                Org.TemplateAr = Item["TemplateAr"] == null ? string.Empty : Item["TemplateAr"].ToString();
                            }
                        }
                        catch (Exception ex)
                        {
