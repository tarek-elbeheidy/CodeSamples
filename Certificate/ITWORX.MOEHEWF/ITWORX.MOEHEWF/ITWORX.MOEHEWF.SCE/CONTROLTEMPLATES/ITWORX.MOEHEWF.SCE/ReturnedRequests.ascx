...
<asp:GridView ID="grd_ReturnedRequests" runat="server" AutoGenerateColumns="false" AllowPaging="true" OnPageIndexChanging="grd_ReturnedRequests_PageIndexChanging"
    ...
    <Columns> 
        ...
        <asp:TemplateField HeaderText="<%$Resources:ITWORX_MOEHEWF_SCE, ReturnDate %>">
            <ItemTemplate>
                <asp:Label ID="lbl_ReturnDate" runat="server" ToolTip='<%# Convert.ToDateTime(Eval("ReturnDate"))!=DateTime.MinValue ? Convert.ToDateTime(Eval("ReturnDate")).QatarFormatedDate() +Convert.ToDateTime(Eval("RecievedDate")).QatarFormatedDateReturnTime():string.Empty %>' Text='<%# Convert.ToDateTime(Eval("ReturnDate"))!=DateTime.MinValue ? Convert.ToDateTime(Eval("ReturnDate")).QatarFormatedDate() +Convert.ToDateTime(Eval("RecievedDate")).QatarFormatedDateReturnTime():string.Empty %>'></asp:Label>
            </ItemTemplate>
        </asp:TemplateField>
