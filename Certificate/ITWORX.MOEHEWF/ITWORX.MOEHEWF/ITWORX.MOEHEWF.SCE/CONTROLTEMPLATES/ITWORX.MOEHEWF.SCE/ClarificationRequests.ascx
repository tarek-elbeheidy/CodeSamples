...
<div class="row margin-top-15">
    ...
    <div class="col-sm-6 text-right">
        <h4 class="font-weight-600 font-size-16 text-right numOfReuq">
...
<asp:GridView ID="grd_ClarRequests" runat="server" AutoGenerateColumns="false" AllowPaging="true" OnPageIndexChanging="grd_ClarRequests_PageIndexChanging"
    ...
    <Columns> 
        <asp:TemplateField HeaderText="<%$Resources:ITWORX_MOEHEWF_SCE, RequestNumber %>"  HeaderStyle-Width="100">
         ...
         <asp:TemplateField HeaderText="<%$Resources:ITWORX_MOEHEWF_SCE, RecievedDate %>">
          ...
          <asp:TemplateField HeaderText="<%$Resources:ITWORX_MOEHEWF_SCE, CertificateHolderQatarID %>">
            <ItemTemplate >
                <asp:Label ID="lbl_HolderQatarID" runat="server" ToolTip='<%#  Eval("CertificateHolderQatarID")%>' Text='<%#  Eval("CertificateHolderQatarID")%>'></asp:Label>
            </ItemTemplate>
        </asp:TemplateField>
           <asp:TemplateField HeaderText="<%$Resources:ITWORX_MOEHEWF_SCE, ApplicantNameAccToCert %>">
            <ItemTemplate>
                <asp:Label ID="lbl_ApplicantNameFromCert" runat="server" ToolTip='<%#  Eval("StudentNameAccToCert")%>'  Text='<%#  Eval("StudentNameAccToCert")%>'></asp:Label>
            </ItemTemplate>
        </asp:TemplateField>
         <asp:TemplateField HeaderText="<%$Resources:ITWORX_MOEHEWF_SCE, Nationality %>">
            <ItemTemplate>
                <asp:Label ID="lbl_Nationality" runat="server" ToolTip='<%#  Eval("Nationality")%>' Text='<%#  Eval("Nationality")%>' ></asp:Label>
            </ItemTemplate>
        </asp:TemplateField>
        <asp:TemplateField HeaderText="<%$Resources:ITWORX_MOEHEWF_SCE, CertificateSource %>">
            <ItemTemplate> 
                <asp:Label ID="lbl_CertificateSource" runat="server" Text='<%#  Eval("Country")%>' ToolTip='<%#  Eval("Country")%>'></asp:Label>
            </ItemTemplate>
        </asp:TemplateField>
        ...
        <asp:TemplateField HeaderText="<%$Resources:ITWORX_MOEHEWF_SCE, ClarificationReason %>">
            <ItemTemplate>
                <asp:Label ID="lbl_RequiredClarRequested" runat="server" Text='<%# Eval("ClarificationReason") %>' ToolTip='<%# Eval("ClarificationReason") %>'></asp:Label>
            </ItemTemplate>
        </asp:TemplateField>
        <asp:TemplateField HeaderText="<%$Resources:ITWORX_MOEHEWF_SCE, ClarRequestedDate %>">
            <ItemTemplate>
                <asp:Label ID="lbl_RequiredClarDate" runat="server" ToolTip='<%# Convert.ToDateTime(Eval("RequestClarificationDate"))!=DateTime.MinValue ?  Convert.ToDateTime(Eval("RequestClarificationDate")).QatarFormatedDate() +Convert.ToDateTime(Eval("RequestClarificationDate")).QatarFormatedDateReturnTime() :string.Empty %>' Text='<%# Convert.ToDateTime(Eval("RequestClarificationDate"))!=DateTime.MinValue ?  Convert.ToDateTime(Eval("RequestClarificationDate")).QatarFormatedDate() +Convert.ToDateTime(Eval("RequestClarificationDate")).QatarFormatedDateReturnTime() :string.Empty %>'></asp:Label>
            </ItemTemplate>
        </asp:TemplateField>
