...
{
    public partial class Procedure_AssistSecretaryListing : UserControlBase
    {
        ...
        private void BindGrid(string ReqID)
        {
            try
            {
                Logging.GetInstance().Debug("Enter Procedure_AssistSecretaryListing.BindGrid");
                List<Entities.Procedures> Procedures = BL.AllProcedures.GetProcedurebyReqID(Utilities.Constants.PAAssistSecretaryProcedures, ReqID);
            ...
            {
                Logging.GetInstance().Debug("Exit Procedure_AssistSecretaryListing.BindGrid");
            }
        }

        protected void grd_Procedures_PageIndexChanging(object sender, GridViewPageEventArgs e)
        {
            Logging.GetInstance().Debug("Enter Procedure_AssistSecretaryListing.grd_Procedures_PageIndexChanging");
            try
            {
            ...
            {
                Logging.GetInstance().Debug("Exit Procedure_AssistSecretaryListing.grd_Procedures_PageIndexChanging");
            }
        }

        protected void lnk_View_Click(object sender, EventArgs e)
        {
            Logging.GetInstance().Debug("Enter Procedure_AssistSecretaryListing.lnk_View_Click");
            try
            {
                if (!IsRefresh)
                {
                    ...
                    SPUtility.Redirect(SPContext.Current.Web.Url + "/Pages/PAProcedureAssistSecretaryDetails.aspx", SPRedirectFlags.DoNotEndResponse, HttpContext.Current);
                }
            }
            catch (Exception ex)
            {
            ...
            {
                Logging.GetInstance().Debug("Exit Procedure_AssistSecretaryListing.lnk_View_Click");
            }
        }
        ...
        protected void lnk_AddNewProcedurePopUp_Click(object sender, EventArgs e)
        {
            ...
            Response.Redirect(SPContext.Current.Web.Url + "/_layouts/15/ITWORX.MOEHEWF.PA/AssistantUndersecretary.aspx");
        }
    }
}