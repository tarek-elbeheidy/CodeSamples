...
namespace ITWORX.MOEHEWF.PA.BL
{
    ...
    {
        public static List<Entities.CalculatedDetailsForCertificate> GetCalculatedDetailsForCertificate(int requestNumber, int LCID)
        {
                            ...
                            if (calculatedCollection != null || calculatedCollection.Count > 0)
                            {
                                foreach (SPListItem item in calculatedCollection)
                                {
                                    calculatedDetailsForCertificate.Add(new Entities.CalculatedDetailsForCertificate()
                                    {
                                        ...
                                        Faculty = item["Faculty"] != null ? Convert.ToString(item["Faculty"]) : string.Empty,
                                        ...
                                        StudyType = new Entities.StudyType()
                                        {
                                            SelectedID = item["StudyingType"] != null ? new SPFieldLookupValue(item["StudyingType"].ToString()).LookupId.ToString() : string.Empty,
                                            SelectedTitle = item["StudyingType"] != null ? (LCID == 1033 ? new SPFieldLookupValue(item["StudyingType"].ToString()).LookupValue : new SPFieldLookupValue(item["StudyingTypeAr"].ToString()).LookupValue) : string.Empty
                                        }
                                        ,
                                        AcceptedHours = item["AcceptedHours"] != null ? Convert.ToString(item["AcceptedHours"]) : string.Empty
                                        ,
                                        StudyingPeriod = item["StudyingPeriod"] != null ? Convert.ToString(item["StudyingPeriod"]) : string.Empty
                                    });
                                }
                            }
                        }
                ...
                SPSecurity.RunWithElevatedPrivileges(delegate ()
                {
                    using (SPSite site = new SPSite(SPContext.Current.Site.Url))
                    {
                        using (web = site.OpenWeb())
                        {
                            ...
                            item["AcceptedHours"] = calculatedDetailsForCertificate.AcceptedHours;
            ...
            try
            {
                SPSecurity.RunWithElevatedPrivileges(delegate ()
                {
                    using (SPSite site = new SPSite(SPContext.Current.Site.Url))
                    {
                        using (web = site.OpenWeb())
                        {
                            SPList universitiesDataList = web.Lists[Constants.PACalculatedDetailsForCertificate];
                        ...
                        using (SPWeb web = site.OpenWeb())
                        {
                            ...
                            if (item != null)
                            {
                                calculatedDetails = new Entities.CalculatedDetailsForCertificate()
                                {
                                    ...
                                    Faculty = Convert.ToString(item["Faculty"]),
                                    StudySystem = new Entities.StudySystem()
                                    {
                                        SelectedID = item["StudyingSystem"] != null ? new SPFieldLookupValue(item["StudyingSystem"].ToString()).LookupId.ToString() : string.Empty,
                                        SelectedTitle = item["StudyingSystem"] != null ? (LCID == 1033 ? new SPFieldLookupValue(item["StudyingSystem"].ToString()).LookupValue : new SPFieldLookupValue(item["StudyingSystemAr"].ToString()).LookupValue) : string.Empty
