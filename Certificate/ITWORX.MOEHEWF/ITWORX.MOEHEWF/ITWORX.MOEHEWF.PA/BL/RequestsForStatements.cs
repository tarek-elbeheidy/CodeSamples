                ...
                using (SPSite site = new SPSite(SPContext.Current.Site.Url))
                {
                    using (SPWeb web = site.OpenWeb())
                    {
                        try
                        {
                            ...
                            SPList list = web.Lists[Utilities.Constants.PAStatementRequests];
                            var q = Common.Utilities.BusinessHelper.GetQueryObject("<Where><Eq><FieldRef Name='RequestID' /><Value Type='Text'>" + reqID +
                                "</Value></Eq></Where> <OrderBy><FieldRef Name='StatementDate' Ascending='False' /></OrderBy>");
