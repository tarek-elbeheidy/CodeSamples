                ...
                using (SPSite site = new SPSite(SPContext.Current.Site.Url))
                {
                    using (SPWeb web = site.OpenWeb())
                    {
                        try
                        {
                            ...
                            SPList list = web.Lists[Utilities.Constants.PADecisions];
                    ...
                    using (SPWeb web = site.OpenWeb())
                    {
                        try
                        {
                            ...
                            var q = Common.Utilities.BusinessHelper.GetQueryObject("<Where><And><Eq><FieldRef Name='RequestID' /><Value Type='Text'>" +
                                ReqID + "</Value></Eq><Eq><FieldRef Name='DecisionMaker' /><Value Type='Text'>" + SPContext.Current.Web.CurrentUser.Name +
                                "</Value></Eq></And></Where><OrderBy><FieldRef Name='DecisionDate' Ascending='False' /></OrderBy>");
                ...
                using (SPSite site = new SPSite(SPContext.Current.Site.Url))
                {
                    using (SPWeb web = site.OpenWeb())
                    {
                        try
                        {
                            ...
                            SPList list = web.Lists[Utilities.Constants.PADecisions];
                            var q = Common.Utilities.BusinessHelper.GetQueryObject("<Where><And><Eq><FieldRef Name='RequestID' /><Value Type='Text'>" +
                                ReqID + "</Value></Eq><Eq><FieldRef Name='SPGroupName' /><Value Type='Text'>" + SPGroupName +
                                "</Value></Eq></And></Where><OrderBy><FieldRef Name='DecisionDate' Ascending='False' /></OrderBy>");
