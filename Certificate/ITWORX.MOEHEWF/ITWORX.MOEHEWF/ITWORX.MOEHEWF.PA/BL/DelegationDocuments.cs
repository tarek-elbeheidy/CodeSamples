...
namespace ITWORX.MOEHEWF.PA.BL
{
    public class DelegationDocuments
    {
        public static Entities.DelegationDocuments GetDocumentByDelegationId(int itemId, SPWeb web)
        {
            ...
            try
            {
                SPList delegationDocumentsLibrary = web.Lists[Utilities.Constants.PADelegationDocuments];
            ...
            try
            {
                SPSecurity.RunWithElevatedPrivileges(delegate ()
                {
                    using (SPSite site = new SPSite(SPContext.Current.Site.Url))
                    {
                        ...
                        {
                            SPList delegationList = web.Lists[Utilities.Constants.PADelegationDocuments];
            ...
            try
            {
                SPSecurity.RunWithElevatedPrivileges(delegate ()
                {
                    using (SPSite site = new SPSite(SPContext.Current.Site.Url))
                    {
                        ...
                        {
                            SPList delegationDocuments = web.Lists[Utilities.Constants.PADelegationDocuments];
                            ...
                            SPQuery documentsQuery = Common.Utilities.BusinessHelper.GetQueryObject("<Where><Eq><FieldRef Name='RequestID' /><Value Type='Lookup'>" + requestNumber + "</Value></Eq></Where>");

                            SPQuery templatesQuery = Common.Utilities.BusinessHelper.GetQueryObject("<FieldRef Name='ID' /><FieldRef Name='Title' /><FieldRef Name='TitleAr' />");
                ...
                SPSecurity.RunWithElevatedPrivileges(delegate ()
                {
                    using (SPSite site = new SPSite(SPContext.Current.Site.Url))
                    {
                        using (web = site.OpenWeb())
                        {
                            ...
                            SPList delegationDocumentist = web.Lists[Utilities.Constants.PADelegationDocuments];
