        ...
        <div class="no-padding row unheighlighted-section margin-2">
            ...
            <Columns>
                ...
                <asp:TemplateField HeaderText="<%$Resources:ITWORX.MOEHEWF.Common, RequestType %>">
                    <ItemTemplate>
                        <asp:HiddenField ID="hdnRegisteredSchool" runat="server" Value= '<%# Eval("CurrentGrade") %>' />
                ...
                <asp:Label ID="lblRequestSubmitDate" runat="server" Text='<%# Convert.ToDateTime(Eval("SubmitDate"))!=DateTime.MinValue ?(  LCID == (int)Language.English?ConvertDateCalendar(Convert.ToDateTime(Eval("SubmitDate")), "Gregorian", "en-US") + ExtensionMethods.QatarFormatedDateReturnTime(Convert.ToDateTime(Eval("SubmitDate"))):ToArabicDigits(ConvertDateCalendar(Convert.ToDateTime(Eval("SubmitDate")), "Gregorian", "en-US")) + ToArabicDigits(ExtensionMethods.QatarFormatedDateReturnTime(Convert.ToDateTime(Eval("SubmitDate")))).Replace("م", "مساء").Replace("ص", "صباحا")) :string.Empty %>'></asp:Label>
